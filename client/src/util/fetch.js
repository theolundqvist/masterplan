...
function fetchCourses(program, year) {
  ...
  return safeFetch(
    ...
    data = data
      ...
      const coursecodes = data.map(x => x.courseCode);
    return data.filter((x, index) => coursecodes.indexOf(x.courseCode) == index);
  });
}
...
function fetchTenta(program, year) {
  ...
  return safeFetch(
    `${__APP_ENV__.SERVER_URL || "http://localhost:1234"
    }/tenta?course=${program}&year=${year}`,
  ).then((data) => {
    return data;
  });
}

export { fetchCourses, fetchPrograms, fetchTenta, fetchYears, safeFetch };
